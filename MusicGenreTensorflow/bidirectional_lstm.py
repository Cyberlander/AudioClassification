...
def load_data():
    ...
    #aranged = list(np.arange( len(x_train)))
    #shuffled_indices = random.sample( aranged, len(aranged))
...
def BiRNN(x):
    ...
    forward_lstm_cell = rnn.BasicLSTMCell( num_hidden, forget_bias=1.0)
    backward_lstm_cell = rnn.BasicLSTMCell( num_hidden, forget_bias=1.0)

    outputs, _, _, = rnn.static_bidirectional_rnn(
        forward_lstm_cell, backward_lstm_cell, x, dtype=tf.float32
    )
    ...
    #prediction_dropout = tf.nn.dropout( prediction, 0.2)
    return prediction
